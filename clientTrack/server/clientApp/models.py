...
class Client(models.Model):
    ...
    def time_breakdown(self):
        unpaid_work = self.session.filter(is_billed=False, end_time__isnull=False)
        breakdown = (
            unpaid_work.values("system__name")
            .annotate(
                total_seconds=Sum(
                    ExpressionWrapper(
        ...
        for item in breakdown:
            ...
            results.append(
                {"system": item["system__name"], "hours": round(total_hours, 2)}
            )
...
class AmountToPay(models.Model):
    ...
    system = models.ForeignKey(
        System, on_delete=models.SET_NULL, null=True, related_name="system_used"
    )
    ...
    def __str__(self):
        ...
        return f'{self.client.full_name} on {system_name} ({self.start_time.strftime("%Y-%m-%d %H:%M")})'
